/*************************************************************************
 *                                                                       *
 *  NOTE: THIS CODE IS IDENTICAL TO THAT OF NPB VERSION 2.3. IT IS NOT   *
...
/*************************************************************************
 *                                                                       * 
 *        N  A  S     P A R A L L E L     B E N C H M A R K S  2.4       *
 *                                                                       * 
 *                                  I S                                  * 
 *                                                                       * 
 ************************************************************************* 
 *                                                                       * 
 *   This benchmark is part of the NAS Parallel Benchmark 2.4 suite.     *
 *   It is described in NAS Technical Report 95-020.                     * 
 *                                                                       * 
 *   Permission to use, copy, distribute and modify this software        * 
 *   for any purpose with or without fee is hereby granted.  We          * 
 *   request, however, that all derived work reference the NAS           * 
 *   Parallel Benchmarks 2.4. This software is provided "as is"          *
 *   without express or implied warranty.                                * 
 *                                                                       * 
 *   Information on NPB 2.4, including the technical report, the         *
...
#if TOTAL_KEYS_LOG_2 > 29
  #define  HEX_TWENTYNINE          (0x100000000/8)
  #define  TOTAL_KEYS          (HEX_TWENTYNINE * (1<< (TOTAL_KEYS_LOG_2-29)))
  #define  MPI_INT_TYPE        MPI_LONG_LONG
...
#if MAX_KEY_LOG_2 > 29
  #define HEX_TWENTYNINE          (0x100000000/8)
  #define MAX_KEY            (HEX_TWENTYNINE * (1<< (MAX_KEY_LOG_2-29))
#else
  #define  MAX_KEY             (1 << MAX_KEY_LOG_2)
#endif

#define  NUM_BUCKETS         (1 << NUM_BUCKETS_LOG_2)
  //numero de procesos es la cantidad de hilos que tiene cada computador
...
void	create_seq( double seed, double a )
{
  ...
  /*  ,MAX_KEY_LOG_2 (HEX_TWENTYNINE * (1<< (MAX_KEY_LOG_2-29)) )*/
}
...
/*  Confirm that neighbor's greatest key value 
    ...
    if( my_rank > 0 )
        if( k > key_array[0] ) {
            printf("Boundary element incorrect on proc %d; k, key = %d, %d\n",
                   my_rank, k, key_array[0]);
            j++;
        }
...
/*  Confirm keys correctly sorted: count incorrectly sorted keys, if any */
    for( i=1; i<total_local_keys; i++ )
        if( key_array[i-1] > key_array[i] ) {
            printf("Internal element incorrect on proc %d; i, km1, k = %d, %d, %d\n",
                    my_rank, i, key_array[i-1], key_array[i]);
            j++;
        }
                ...
                case 'S':
                    if( i <= 2 )
                    {
                        if( key_buff_ptr[k-1] != test_rank_array[i]+iteration )
                        {
                            ...
                            passed_verification++;
                            printf("Verified: test_rank_array[%d]: %d\n",i,
                                    key_buff_ptr[k-1]-iteration);
                    }
                    else
                    {
                        if( key_buff_ptr[k-1] != test_rank_array[i]-iteration )
...
/*  The final printout  */
    ...
    tiempo=finish-start;
    time_total+=tiempo;
    printf("el hilo %d y el time %f:\n ",my_rank,tiempo);
    MPI_Finalize();
    //printf("tiempo total ",time_total);
}
